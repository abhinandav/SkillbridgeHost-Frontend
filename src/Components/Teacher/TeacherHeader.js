    ...
    <div>
      ...
      <nav className="h-7 text-center rounded-md bg-orange-500 m-4">
        <span className="text-white font-bold text-sm">
          Welcome Teacher!! Lets Fill This World With Creativity
        </span>
      </nav>
      {/* Navbar */}
        ...
        <div className="bg-white shadow">
          <div className="container mx-auto px-4">
            <div className="flex items-center justify-between py-2">
              <div className='sm:flex sm:items-center ml-5 '>
                <svg xmlns="http://www.w3.org/2000/svg" className="w-10 h-10 text-orange-500" viewBox="0 0 24 24">
                ...
                <span className='ml-5 text-md font-semibold hover:text-orange-500 mr-4'><Link className="nav-link" to='/teacher'>HOME</Link></span>
              </div>
              ...
              {(authentication_user.isAuthenticated  && authentication_user.isTeacher   && !authentication_user.isAdmin) ? (<>
                  <Link className="nav-link" to='teacher_profile'>
                    {authentication_user.name}
                  </Link>
                   <button onClick={logout} className="text-gray-800 text-sm font-semibold border px-4 py-2 rounded-lg hover:text-orange-500 hover:border-orange-500 ml-5">
                     Log Out
                   </button>
